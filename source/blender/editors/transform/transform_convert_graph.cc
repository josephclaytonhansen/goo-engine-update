...
static blender::Vector<BeztMap> bezt_to_beztmaps(BezTriple *bezts, int totvert)
{
  ...
  blender::Vector<BeztMap> bezms;
  bezms.resize(totvert);

  /* Assign beztriples to beztmaps. */
  for (int i = 0; i < totvert; i++, prevbezt = bezt, bezt++) {
...
static void sort_time_beztmaps(const blender::MutableSpan<BeztMap> bezms)
{
  ...
  for (BeztMap &bezm : bezms) {
     ...
     * that is out of place gets resolved during sorting. */
    if ((bezm.bezt->vec[0][0] > bezm.bezt->vec[1][0]) &&
        (bezm.bezt->vec[2][0] < bezm.bezt->vec[1][0]))
    {
      bezm.swap_handles = true;
    }
  }
