...
{
  /* Show light object selector using the node's id pointer - same as tex_coord */
                                      ...
                                      GPUNodeStack *out)
{
  ...
  if (node->id != nullptr) {
    ...
    if (light_ob->type == OB_LAMP && light_ob->data != nullptr) {
  ...
  /* Call the GLSL shader function that already exists */
